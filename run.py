...
def get_all_ways_from_hall(positions: str) -> Generator[tuple[int, str], Any, None]:
    for i in range(11):
        ...
        room_is_ready = all(positions[idx] in ('.', letter) for idx in target_room_indices)
...
def is_final_state(position: str) -> bool:
    if labyrinth_depth == 2:
        return position[11:13] == "AA" and position[13:15] == "BB" and position[15:17] == "CC" and position[
            17:19] == "DD"
    return position[11:15] == "AAAA" and position[15:19] == "BBBB" and position[19:23] == "CCCC" and position[
        23:27] == "DDDD"


@lru_cache(maxsize=None)
def get_all_ways_cached(positions: str) -> Tuple[Tuple[int, str], ...]:
    moves = tuple(get_all_ways_from_hall(positions)) + tuple(get_all_ways_from_rooms(positions))
    return moves
...
def solve(lines: list[str]) -> int:
    ...
    while pq:
        ...
        for cost_add, new_pos in get_all_ways_cached(pos):
